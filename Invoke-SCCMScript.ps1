function Invoke-SCCMScript {
    ...
    if ($InputParameters.Count -le 0) {
        ...
        $Parameters.ScriptParameters.ChildNodes | foreach {
            ...
            $InnerParametersXML = "$InnerParametersXML<ScriptParameter ParameterGroupGuid=`"$ParameterGroupGUID`" ParameterGroupName=`"PG_$ParameterGroupGUID`" ParameterName=`"$($ParamName)`" ParameterType=`"$($_.Type)`" ParameterValue=`"$($Value)`"/>"
        }
